...
public class SoftwareUpdateFileTest extends TestBase {
	...
	public void constructor_test() throws IOException {
		...
		final var digest = EncryptionUtils.getFileDigest(file);
	...
	public void compareTo_test() throws IOException {
		...
		final var update2 = FileDetails.parse(file2);
		final var softwareUpdateFile2 = new SoftwareUpdateFile(update2);
		...
		final var update3 = FileDetails.parse(file3);
		final var softwareUpdateFile3 = new SoftwareUpdateFile(update3);
		...
		final var update5 = FileDetails.parse(file5);
		final var softwareUpdateFile5 = new SoftwareUpdateFile(update5);
		...
		final var update6 = FileDetails.parse(file6);
		final var softwareUpdateFile6 = new SoftwareUpdateFile(update6);
	...
	public void buildGridPane_test() throws IOException, HederaClientException {
		...
		final var gridPane = softwareUpdateFile.buildGridPane();
		...
		final var hBoxChildren = ((HBox) gridPane.getChildren().get(6)).getChildren();
		...
		final var metadataActions = new MetadataFile(softwareUpdateFile.getName()).getMetadataActions();
		final var digest = metadataActions.get(0).getUserComments();
		assertEquals(digest, softwareUpdateFile.getDigest().replace("\u00a0", " ").replace("\n", " "));

		final var gridPaneHistory = softwareUpdateFile.buildGridPane();

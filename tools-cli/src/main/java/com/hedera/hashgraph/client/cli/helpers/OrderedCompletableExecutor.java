...
public class OrderedCompletableExecutor extends CompletableExecutor {
    private static final Logger LOGGER = LogManager.getLogger(OrderedCompletableExecutor.class);

    private final Map<FileId, OrderedTask> taskMap = new HashMap<>();
    private OrderedTask lastTask;

    public OrderedCompletableExecutor(final Executor executor) {
        super(executor);
    }
...
//        if (delegate.isShutdown()) return;
        ...
        if (orderedTask == null) {
            ...
            orderedTask = new OrderedTask(executor);
            taskMap.put(fileId, orderedTask);
        }
//TODO this will return a response, the receipt comes later, in 'get', so whencomplete needs to be done differently
        final var future = orderedTask.addCompletableTask(task);
        //TODO
...
//                results.add(task.getResult(receipt));
    ...
    private static class OrderedTask {
        private final Map<Object, CompletableFuture<TransactionResponse>> nodeMap;
        private final Executor executor;
        private CompletableFuture<TransactionReceipt> receipt;

        public OrderedTask(final Executor executor) {
            this.executor = executor;
            nodeMap = new HashMap<>();
        }

        public CompletableFuture<TransactionResponse> addCompletableTask(final CompletableTask task) {
            //TODO
            ...
            final var newFuture = previousFuture.thenApplyAsync(response -> {
                try {
                    return task.submitTransaction();
                } catch (Exception ex) {
                    LOGGER.error("stuff", ex);
                    throw new CompletionException(ex);
                }
            });
            nodeMap.put(node, newFuture);

            receipt = newFuture.thenApplyAsync(response -> {
                try {
        ...
        public TransactionReceipt get() throws InterruptedException, ExecutionException {
            return receipt.get();
        }
    }
}
